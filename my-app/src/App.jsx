...
function buildMonthly(transactions, year = 2024) {
  const map = {};
  for (let m = 1; m <= 12; m++) {
    const k = `${year}-${String(m).padStart(2, "0")}`;
    map[k] = { key: k, year, label: MN[m - 1], full: MNF[m - 1], income: 0, expenses: 0, txs: [] };
  }
...
// ─── TREND CHART ─────────────────────────────────────────────────────────────
function TrendChart({ monthly, selectedMonth, onMonthClick, year, onYearChange }) {
  ...
  const hint = selectedMonth
    ? `Viewing ${monthly.find((m) => m.key === selectedMonth)?.full} ${year} — click same or another month`
    : `Click any month on the chart to see its breakdown for ${year}`;
  ...
  return (
    <div className="anim-fade-up-2" ref={wrapRef} onMouseMove={handleMouseMove}
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 4, flexWrap: "wrap", gap: 10 }}>
        <div>
          <div style={{ fontFamily: "var(--font-display)", fontSize: 17, color: "var(--text)" }}>Balance Trend — FY {year}</div>
          <div style={{ fontSize: 12, color: "var(--text3)", marginTop: 4 }}>{hint}</div>
        </div>
        <div style={{ display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 10px", borderRadius: 10, border: "1px solid var(--glass-border)", background: "var(--glass)" }}>
            <button type="button" onClick={() => onYearChange(year - 1)} style={{ background: "none", border: "none", color: "var(--text2)", cursor: "pointer", fontSize: 14, padding: "0 4px" }}>‹</button>
            <span style={{ fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--gold)" }}>{year}</span>
            <button type="button" onClick={() => onYearChange(year + 1)} style={{ background: "none", border: "none", color: "var(--text2)", cursor: "pointer", fontSize: 14, padding: "0 4px" }}>›</button>
          </div>
      ...
      {tooltip && (() => {
        ...
        return (
          <div style={{
            ...
            <div style={{ fontWeight: 600, color: "var(--text)", marginBottom: 9, fontSize: 13, borderBottom: "1px solid var(--glass-border)", paddingBottom: 7 }}>{m.full} {m.year}</div>
  ...
  return (
    <div className="anim-slide-down" id="month-panel" style={{
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontFamily: "var(--font-display)", fontSize: 18, color: "var(--gold)" }}>{m.full} {m.year}</div>
...
export default function App() {
  ...
  const trendMonthly = buildMonthly(transactions, trendYear);
        ...
        <div style={{ marginLeft: 240, flex: 1, minHeight: "100vh" }}>
          ...
          <div style={{ padding: "28px 32px" }}>
            {tab === "dashboard" && (
              <>
                <StatCards d={d} />
                <TrendChart monthly={trendMonthly} selectedMonth={selectedMonth} onMonthClick={handleMonthClick} year={trendYear} onYearChange={handleYearChange} />
                {selectedMonth && <MonthPanel monthly={trendMonthly} selectedMonth={selectedMonth} onClose={() => setSelectedMonth(null)} />}
